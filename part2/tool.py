...
## TO DO ##
def plot_learning_curve(data, axis, title):
    """_summary_
    ...
    epoch = [i for i in range(len(data))]
    plt.plot(epoch, data)
    plt.title(title)
    plt.ylabel(axis)
...
def train(model, train_loader, val_loader, num_epoch, log_path, save_path, device, criterion, scheduler, optimizer):
    ...
    for i in range(num_epoch):
        ...
        with torch.no_grad():
            ...
            # averaging val_loss and calculate accuracy
    ...
    plot_learning_curve(overall_acc, 'Accuracy', 'Training Accuracy')
    plot_learning_curve(overall_loss, 'Loss', 'Training Loss')
    plot_learning_curve(overall_val_acc, 'Accuracy', 'Validation Accuracy')
    plot_learning_curve(overall_val_loss, 'Loss', 'Validation Loss')
    
    print("Done training & Saving curves")
